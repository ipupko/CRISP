...
def draw_scatter(ax, df_plot: pd.DataFrame,
    ...
    # track placed label positions for collision resolution
    placed_positions = []

    # Overlay anomalies with star markers, arrows and ID labels
    for atype, mask in anomaly_masks.items():
        ...
        col        = ANOMALY_COLORS.get(atype, '#ffcc00')
        xlim       = ax.get_xlim()
        ylim       = ax.get_ylim()
        ...
        for _, row in grp.iterrows():
            if label_field == "FID_IID":
                lbl = f"{row['FID']}_{row['IID']} ({type_label})"
            elif label_field == "FID":
                lbl = f"{row['FID']} ({type_label})"
            else:
                lbl = f"{row['IID']} ({type_label})"
            ...
            candidates = [
                ...
                ( x_range*0.10, -y_range*0.10),
                (-x_range*0.10, -y_range*0.10),
                ( x_range*0.15,  y_range*0.05),
                (-x_range*0.15,  y_range*0.05),
                ( x_range*0.05,  y_range*0.18),
                ( x_range*0.05, -y_range*0.18),
            ...
            for dx, dy in candidates:
                ...
                # boundary check
                if tx < xlim[0] + margin_x: continue
                if tx > xlim[1] - margin_x: continue
                if ty < ylim[0] + margin_y: continue
                if ty > ylim[1] - margin_y: continue

                # collision check
                overlap = False
                for px, py in placed_positions:
                    if abs(tx-px) < x_range*0.09 and abs(ty-py) < y_range*0.07:
                        overlap = True; break
                if not overlap:
                    best_xy = (tx, ty); break

            # fallback: clamp first candidate to bounds
            if best_xy is None:
                dx, dy = candidates[0]
                tx = float(np.clip(row['F']     + dx,
                                   xlim[0]+margin_x, xlim[1]-margin_x))
                ty = float(np.clip(row['YCOUNT'] + dy,
                                   ylim[0]+margin_y, ylim[1]-margin_y))
                best_xy = (tx, ty)

            placed_positions.append(best_xy)
