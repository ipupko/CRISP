#!/usr/bin/env python3
"""
CRISP: plot_sexcheck.py
=======================
Sex check and aneuploidy detection plotting script (Python engine).

Generates F-statistic vs Y-count scatter plots and detects sex
chromosome anomalies from PLINK sex check output.

Developed by Igor Pupko
https://github.com/ipupko/CRISP
Date Created : 08/02/2026
Date Updated : 17/03/2026
...
Usage
-----
    python3 plot_sexcheck.py \\
        <input_dir> <output_dir> <f_stat_y_count_file> \\
        <f_female_max> <f_male_min> <f_turner> \\
        <f_klinefelter> <f_xxx> <y_use_mean> <y_manual>

Arguments
---------
input_dir           Input directory (unused, kept for R parity)
output_dir          Output directory for plots and reports
f_stat_y_count_file Merged F-stat and Y-count TSV file
f_female_max        F threshold upper bound for females (default 0.2)
f_male_min          F threshold lower bound for males (default 0.8)
f_turner            F threshold for Turner X0 detection (default 0.8)
f_klinefelter       F threshold for Klinefelter XXY detection (default 0.8)
f_xxx               F threshold for triple-X detection (default -0.15)
y_use_mean          Use mean Y count as threshold YES or NO
y_manual            Manual Y count threshold if y_use_mean is NO
"""
