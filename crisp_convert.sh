...
###
### Chromosome cleaning keeps 1-22, X, Y, XY, MT only.
### Non-standard chromosomes are excluded and reported.
### Multiallelic variants excluded via --max-alleles 2 (VCF and BGEN).
###
### Usage:
###   bash crisp_convert.sh
...
if [ ! -f "${INSTRUCTION_FILE}" ]; then
    ...
    echo "[CRISP]        Usage: bash crisp_convert.sh --config <path>"
    exit 1
fi
...
REPORT_FILE="${OUTPUT_DIR}/${PROJECT_NAME}_step2_conversion_report.txt"
EXTRACT_LIST="${CONVERTED_DIR}/chr_extract_list.txt"

echo "[CRISP] Output : ${CONVERTED_DIR}"
echo "[CRISP] Log    : ${LOG_FILE}"
echo ""
...
case "${INPUT_FORMAT^^}" in
    ...
    PLINK)
        echo "[CRISP] PLINK input, creating working copy..."
        echo ""
        ${PLINK1} \
            --bfile "${INPUT_PATH}" \
            --make-bed \
    ...
    PED)
        echo "[CRISP] Converting PED/MAP to BED/BIM/FAM..."
        echo ""
        ${PLINK1} \
            --ped "${INPUT_PATH}.ped" \
            --map "${INPUT_PATH}.map" \
    ...
    VCF)
        # resolve the actual file path, try plain, .vcf.gz, .vcf
        if [ -f "${INPUT_PATH}" ]; then
            ...
            echo "[CRISP]        Tried:"
            echo "[CRISP]          ${INPUT_PATH}"
            echo "[CRISP]          ${INPUT_PATH}.vcf.gz"
            echo "[CRISP]          ${INPUT_PATH}.vcf"
            exit 1
        fi

        echo "[CRISP] Converting VCF to BED/BIM/FAM (PLINK 2)..."
        echo "[CRISP] File: ${VCF_FILE}"
        echo ""
        ${PLINK2} \
            --vcf "${VCF_FILE}" \
            --make-bed \
    ...
    BCF)
        ...
        echo "[CRISP] Converting BCF to BED/BIM/FAM (PLINK 2)..."
        echo "[CRISP] File: ${BCF_FILE}"
        echo ""
        ${PLINK2} \
            --bcf "${BCF_FILE}" \
            --make-bed \
    ...
    BGEN)
        ...
        # need the companion .sample file
        ...
        echo "[CRISP] Converting BGEN to BED/BIM/FAM (PLINK 2)..."
        echo "[CRISP] File: ${BGEN_FILE}"
        echo ""
        ${PLINK2} \
...
echo "[CRISP] Variants : ${VARIANTS_BEFORE}"
echo "[CRISP] Samples  : ${SAMPLES}"
echo ""

##########################################################################
### STEP 2B: CHROMOSOME CLEANING
### Keep 1-22, X, Y, XY, MT. Anything else gets dropped.
##########################################################################
...
echo "[CRISP] Checking chromosome composition..."
echo ""

awk '{print $1}' "${OUT_PREFIX}.bim" | sort | uniq -c | sort -k2,2 \
    > /tmp/crisp_chr_counts.txt
...
chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chrX chrY chrXY chrMT"
...
if [ "${non_standard_found}" -eq 0 ]; then
    echo "[CRISP] All chromosomes are standard. Skipping chr cleaning."
    # point CLEAN_PREFIX at the unconverted output and keep counts consistent
    ...
    echo "[CRISP] Variants retained : ${VARIANTS_AFTER}"
    echo "[CRISP] Variants excluded : ${excluded_total} (non-standard chromosomes)"
    echo ""
    ...
    if [ "${KEEP_INTERMEDIATE^^}" = "NO" ]; then
        echo "[CRISP] Removing intermediate files..."
...

{
    ...
    echo "  CONVERSION"
    case "${INPUT_FORMAT^^}" in
        PLINK) echo "  Tool : PLINK 1.9 (pass-through)" ;;
        PED)   echo "  Tool : PLINK 1.9" ;;
        *)     echo "  Tool : PLINK 2" ;;
    esac
...
echo ""
echo "[CRISP] Step 2 complete. Output prefix: ${CLEAN_PREFIX}"
echo ""
