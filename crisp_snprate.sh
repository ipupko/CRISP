...
write_report() {
...

    {
        ...
        echo "------------------------------------------------------------------"
        echo "  DIAGNOSTIC COUNTS (pre-filtering)"
        echo "  Total variants               : ${DIAG_TOTAL}"
        echo "  Monomorphic SNPs (MAF = 0)   : ${DIAG_MONO}"
        echo "  SNPs below MAF ${MAF}          : ${DIAG_BELOW_MAF}"
        echo "  SNPs passing both filters    : ${DIAG_PASSING}"
        echo "------------------------------------------------------------------"
...
##########################################################################
### STEP 4b: DIAGNOSTIC COUNTS (PRE-FILTERING)
### Count monomorphic SNPs and SNPs below MAF threshold before
### any removal is applied. Gives user full picture upfront.
##########################################################################
...
# Resolve final call rate output prefix
FINAL_CALLRATE_PREFIX=$(ls -t "${STEP4_DIR}"/${PROJECT_NAME}*.bed 2>/dev/null \
    | head -1 | sed 's/\.bed$//')

if [ -z "${FINAL_CALLRATE_PREFIX}" ]; then
    echo "[CRISP] WARNING: Could not resolve call rate output for diagnostics."
    DIAG_PREFIX="${INPUT_PREFIX}"
else
    DIAG_PREFIX="${FINAL_CALLRATE_PREFIX}"
fi

DIAG_LOG="${LOG_DIR}/step4_diagnostics.log"

echo "[CRISP] Running diagnostic missingness and frequency check..."

# Generate frequency file for diagnostics
${PLINK1} \
    --bfile "${DIAG_PREFIX}" \
    --freq \
...
# Count monomorphic SNPs (MAF = 0)
if [ -f "${FREQ_FILE}" ]; then
    DIAG_MONO=$(awk 'NR>1 && $5==0' "${FREQ_FILE}" | wc -l | tr -d '[:space:]')
    DIAG_BELOW_MAF=$(awk -v maf="${MAF}" 'NR>1 && $5<maf && $5>0' "${FREQ_FILE}" \
        | wc -l | tr -d '[:space:]')
...
echo ""
echo "[CRISP] DIAGNOSTIC COUNTS (pre-filtering)"
echo "        Total variants               : ${DIAG_TOTAL}"
echo "        Monomorphic SNPs (MAF = 0)   : ${DIAG_MONO}"
echo "        SNPs below MAF ${MAF}          : ${DIAG_BELOW_MAF}"
echo "        SNPs passing both filters    : ${DIAG_PASSING}"
...
##########################################################################
### STEP 4c: MONOMORPHIC SNP REMOVAL (OPTIONAL)
### Applied after call rate filtering and diagnostics.
...
##########################################################################
### STEP 4d: MAF FILTERING (OPTIONAL)
### Applied after monomorphic removal.
### Removes variants below MAF threshold.
### Controlled by FILTER_MAF and MAF in crisp_instructions.txt.
##########################################################################
...
if [ "${FILTER_MAF^^}" = "YES" ]; then
    ...
    echo "[CRISP] Applying MAF filter: ${MAF}"
    echo "[CRISP] Running: ${PLINK1} --bfile ${MAF_IN} --maf ${MAF} --make-bed --out ${MAF_OUT}"
    echo ""
    ...
    echo ""
    echo "[CRISP] Variants before MAF filter : ${VARIANTS_PRE_MAF}"
    echo "[CRISP] Variants removed (MAF)     : ${MAF_REMOVED}"
    echo "[CRISP] Variants after MAF filter  : ${VARIANTS_AFTER_MAF}"
    echo ""

    # Separate MAF exclusion list
    awk '{print $2}' "${MAF_IN}.bim"  | sort > /tmp/crisp_pre_maf.txt
    awk '{print $2}' "${MAF_OUT}.bim" | sort > /tmp/crisp_post_maf.txt
    comm -23 /tmp/crisp_pre_maf.txt \
             /tmp/crisp_post_maf.txt > "${MAF_EXCL_LIST}"
    ...
    if [ "${KEEP_INTERMEDIATE^^}" = "NO" ] && \
       [ "${MAF_IN}" != "${INPUT_PREFIX}" ]; then
        ...
        echo "[CRISP] Intermediate pre-MAF files removed."
        echo ""
    fi
    ...
    echo "[CRISP] FILTER_MAF = NO -- MAF filtering skipped."
    echo ""
